 ...
 * email is registered — otherwise the response itself would let anyone
...
const forgotPassword = async (req, res) => {
  try {
    ...
    const otp = Math.floor(100000 + Math.random() * 900000);
    const salt = await bcrypt.genSalt(10);
    user.resetPasswordOtp = await bcrypt.hash(otp.toString(), salt);
    ...
    try {
      await sendMail({
        to: email,
        subject: "Reset your Conversa password - " + otp,
        html: `<!DOCTYPE html>
  <html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>Reset your Conversa password</title>
  </head>
  <body style="margin:0;padding:0;background-color:#f0f2f5;font-family:'Segoe UI',Helvetica,Arial,sans-serif;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f0f2f5;padding:40px 0;">
    <tr>
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <td style="padding:40px 40px 32px;">
          <p style="margin:0 0 8px;font-size:15px;color:#374151;">Hello,</p>
          <p style="margin:0 0 28px;font-size:15px;color:#374151;line-height:1.6;">
          We received a request to reset your Conversa password. Use the code below to choose a new one.
          </p>
          <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td align="center" style="background-color:#f5f3ff;border:2px dashed #8b5cf6;border-radius:10px;padding:24px;">
            <p style="margin:0 0 6px;font-size:12px;color:#6b7280;letter-spacing:1px;text-transform:uppercase;">Reset Code</p>
            <p style="margin:0;font-size:42px;font-weight:800;letter-spacing:10px;color:#6366f1;">${otp}</p>
            </td>
          </tr>
          </table>
          <p style="margin:24px 0 0;font-size:13px;color:#6b7280;text-align:center;">
          This code is valid for <strong style="color:#374151;">15 minutes</strong>.
          </p>
        </td>
        </tr>
        <tr>
      ...
      // Don't leak the send failure to the client — same reasoning as the
      // generic response above. It's logged so it's still debuggable.
      console.error("Mail error (forgot-password):", err.message);
    }
 ...
 * Consumes the reset code — valid for one use, same as email verification.
 */
const resetPassword = async (req, res) => {
  try {
    ...
    // Reset also revokes any existing session — a leaked old password
    // shouldn't leave a still-valid refresh token lying around.
